...
Kratos 一套由知名大厂 bilibili 出品的轻量级 Go 微服务框架，包含大量微服务相关框架及工具。

Kratos 团队致力于提供完整的微服务研发体验，整合相关框架及工具后，微服务治理相关部分可对整体业务开发周期无感，从而更加聚焦于业务交付。对每位开发者而言，整套 Kratos 框架也是不错的学习仓库，可以了解和参考到微服务方面的技术积累和经验。
...
#### protobuf
Protobuf，全名是Protocol Buffers，是一种轻便且高效的结构化数据存储格式，由Google开发。Protobuf 可以用于序列化结构化数据，比如XML、JSON等，但是它更小、更快、也更简单。主要被用于通信协议、数据存储等领域的语言无关、平台无关、可扩展的序列化结构数据格式。使用 Protobuf 需要定义数据结构，这些数据结构以 .proto 文件形式保存。这些文件非常简洁，可用于自动化代码生成。

kratos 设计思想就是通过 protobuf 来支持高度自由的定制化，框架制定接口规范，然后通过插件来实现具体需求，实现高度可拔插的微服务框架，企业在开发时可以选择框架已经提供的插件实现，也可以自己定制插件，实现了高度的可定制。并且在 kratos v2 版本中 API定义、gRPC Service、HTTP Service、请求参数校验、错误定义、Swagger API json、应用配置模版等都是基于 Protobuf IDL 来构建的。

![](https://raw.githubusercontent.com/mouuii/picture/master/%E6%88%AA%E5%B1%8F2023-06-15%20%E4%B8%8A%E5%8D%8810.48.50.png)

#### Transport HTTP/gRPC

kratos 框架对传输层进行了抽象，用户也可以实现自己的传输层，框架默认实现了 gRPC 和 HTTP 两种通信协议传输层。Transport 主要的接口：
